-- =============================================================================
-- Phase 2 RPC 適用（get_couple_home_world エラー回避版）
-- Supabase SQL Editor にこのファイル全文を貼って 1 回で Run してください。
-- =============================================================================
...
-- 旧 RPC を削除（戻り値型・引数が変わるものは DROP 必須）
DROP FUNCTION IF EXISTS public.get_couple_home_world() CASCADE;
DROP FUNCTION IF EXISTS public.service_claim_couple_home_world_generation(uuid);
DROP FUNCTION IF EXISTS public.service_upsert_couple_home_world(uuid, text, text, jsonb, uuid[], int, text, text);

-- 以降: supabase/migrations/20260525000000_couple_home_world_phase2.sql の
-- get_couple_home_world 以降をそのまま実行
